...
dados_mensais |> 
  ...
  rename(Veículos = veiculos, Automóvel = automovel, Motocicleta = motocicleta) |> 
  pivot_longer(-data) |> 
...
A análise de séries temporais é efetuada a partir da regressão de um modelo de melhor ajuste sob uma sequência de observações de um evento em um determinado intervalo de tempo de unidades uniformes, sendo estas séries representadas em função do tempo como sua única variável preditora. Entre as vantagens de modelos de série temporal estão o fato de não necessitarem de outras variáveis e de serem capazes de prever grandes períodos a frente, apesar de serem mais dificilmente ajustados em comparação a modelos determinísticos convencionais.
...
#| fig.width: 7
#| fig.height: 5
#| fig.cap: Métricas de erro para modelo SARIMA
...
Foi utilizado o método de Holt-Winters devido ao fato de ser considerado o mais ideal para tratamento de dados sazonais e não estacionários. A @fig-es e a @fig-eres demonstram a visualização do desempenho do modelo e suas métricas de erro:
...
#| fig.width: 7
#| fig.height: 5
#| fig.cap: Métricas de erro para modelo de suavização exponencial
...
Conforme o procedimento de modelagem, a regressão linear é ajustada e representada na @fig-erlinreg e @fig-linreg:
...
#| fig.width: 7
#| fig.height: 5
#| fig.cap: Métricas de erro para regressão linear mensal
...
#| fig.width: 7
#| fig.height: 5
#| fig.cap: Métricas de erro para modelo Random Forest
...
A variedade de modelos testados foram avaliados individualmente, mas diversas métricas de erros utilizados são não normalizados, necessitando, portanto de uma análise comparativa do desempenho de cada técnica em relação às demais. A tabela a seguir apresenta as métricas para cada algoritmo utilizado até o momento, o que auxiliará na escolha de um modelo definitivo:
...
#| fig.width: 7
#| fig.height: 5
#| fig.cap: Tabela de métricas para todos os modelos
...
bind_rows(tbl_metrics1,tbl_metrics2,tbl_metrics3,tbl_metrics4,tbl_metrics5) |> 
  ...
  tab_header("Métricas de Erros",
             subtitle = "Avaliação de desempenho para os modelos preditivos") |> 
...
pred_mensal_2022 |> 
  ggplot(aes(data, mortes)) +
    geom_ribbon(aes(ymax = .pred_upper, ymin = .pred_lower), fill = "gray70", alpha = 0.7) +
    geom_line(aes(color = "Mortes observadas"), linewidth = 1) +
    geom_line(aes(y = .pred, color = "Mortes previstas"), linewidth = 1) +
    scale_color_manual(values = c(
      "Mortes previstas" = onsv_palette$yellow,
...
bind_rows(
  ...
  tab_header("Previsões para 2022",
             subtitle = "Valores estimados para número de mortes por cada modelo") |> 
  fmt_number(columns = c(Previsão,`Limite Mín.`,`Limite Máx.`),
             sep_mark = ".",
             dec_mark = ",") |>
  tab_options(column_labels.background.color = onsv_palette$blue,
              column_labels.font.weight = "bold") |> 
