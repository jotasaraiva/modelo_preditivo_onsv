...
Outra variável que deve ser analisada individualmente é o número de condutores, que possui uma alta correlação com a população e um comportamento similar em questão da sua contribuição ao modelo. Para verificar se o número de condutores possui um acréscimo homogêneo ao longo do período estudado, calcula-se a quantidade de novos condutores por ano, como visto na @fig-newcond, denotando como o ano de 2020 sofreu uma brusca queda de novos indivíduos, possivelmente devido aos impactos da pandemia. Em relação à variável população, os condutores oferecem melhores valores nas métricas de funcionamento.
...
A gravidade do número de mortes e a qualidade da predição podem ser transparecidos pelo cálculo das taxas de mortos por habitantes e por veículos, com o propósito de observar suas tendências. A @fig-rate1 explicita a crescente taxa de mortes no trânsito em função da população:
...
taxas_pred |> 
  ...
  labs(x = NULL, y = NULL) +
  scale_discrete_onsv(
    labels = c("Mortes por 100.000 habitantes", 
               "Mortes previstas por 100.000 habitantes"))
```

A taxa de mortes por veículos, como mostrada na @fig-rate2, demonstra uma queda progressiva significativa, um possível indício do aumento desproporcional da frota de veículos em relação as mortes no trânsito:
...
taxas_pred |>
  ...
  labs(x = NULL, y = NULL) +
  scale_discrete_onsv(
    labels = c("Mortes por 10.000 veículos", 
               "Mortes previstas por 10.000 veículos"))
```
