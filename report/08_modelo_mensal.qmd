...
```{r}
es_model <- HoltWinters(ts)

plot(ts)
lines(es_model$fitted[ ,1], lty = 2, col = "blue")
```

```{r}
es_pred <- predict(es_model, 12, prediction.interval = T, level = 0.95)

plot(ts)
lines(es_model$fitted[ ,1], lty = 2, col = "blue")
lines(es_pred[,1], col = "red")
lines(es_pred[,2], lty=2, col="orange")
lines(es_pred[,3], lty=2, col="orange")
```
