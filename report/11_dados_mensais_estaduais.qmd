...
df_split <- df_nest$data |> 
  lapply(initial_split, prop = 0.8) |> 
  tibble() |> 
  rename(splits = everything()) |> 
  bind_cols(df_nest) |> 
  mutate(
    training = lapply(splits, training),
    testing = lapply(splits, testing)
  ) |> 
  relocate(c(regiao,data))
