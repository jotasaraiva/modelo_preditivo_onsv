...
plot_taxas <- df_total |>
  ...
  mutate(
    mortes = if_else(ano == 2022, 33894, mortes),
    populacao = if_else(ano == 2022, 203080756, populacao)
  ) |>
  mutate(
...
A presente seção inclui o processo e resultado da estimativa de custos para a quantidade de óbitos prevista para 2023, conforme o modelo linear anual previamente ajustado (34.631 óbitos). Em um primeiro momento, faz-se a estimativa dos custos financeiros (@sec-custos-ipea), com base na estimativa dos custos de sinistros do Brasil elaborada pelo Instituto de Pesquisa Econômica Aplicada (IPEA). Em seguida, a @sec-internacoes inclui uma análise da ocupação de leitos voltados ao atendimento dos feridos em sinistros de trânsito, também para o ano de 2023.
...
Por fim, multiplicando o custo individual das mortes em 2023 pelo valor estimado de `r scales::number(pred_2023, big.mark = ".")`, tem-se que o custo total dos óbitos previstos no trânsito em 2023 é de `r custo_rs`, ou aproximadamente **R$ 25,2 bilhões**.
...
media_leitos <- leitos$Quantidade_existente |> mean() |> round(digits = 0)
