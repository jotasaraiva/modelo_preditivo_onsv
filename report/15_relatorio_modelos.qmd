...
sarima_pred <- sarima |> 
  ...
  data.frame() |> 
  rownames_to_column() |> 
  mutate(data = my(rowname)) |> 
  select(-rowname) |> 
  full_join(df_mortes) |> 
  arrange(data)

metricas_sarima <- cbind(ts, ets = sarima$fitted) |> 
  data.frame() |> 
  metricas(truth = ts, estimate = ets)
```
...
sarima_pred |> 
  pivot_longer(c(mortes, Point.Forecast), 
               names_to = "Tipo", values_to = "Mortes") |> 
  mutate(Tipo = if_else(Tipo == "mortes", 
                        "Óbitos reais", "Óbitos previstos")) |> 
  ggplot(aes(data, Mortes)) +
    geom_ribbon(aes(ymin = Lo.80, ymax = Hi.80), fill = "grey80", alpha = 0.5) +
    geom_ribbon(aes(ymin = Lo.95, ymax = Hi.95), fill = "grey90", alpha = 0.5) +
