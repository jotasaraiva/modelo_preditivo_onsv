...
format_billion <- function (x) { paste("R$", format(round(x / 1e9, 1), decimal.mark = "."), "Bi") }
```

- O custo financeiro dos `r format(pred_2023, big.mark = ".", decimal.mark = ",")` óbitos previstos pelo modelo anual linear para 2023 foi estimado com base nos custos médios elaborados pelo Instituto de Pesquisa Econômica Aplicada (IPEA) em @carvalhoCustosAcidentesTransito2020a;

- O estudo do IPEA aborda custos sobre sinistros em aglomerados urbanos, rodovias estaduais e municipais, e rodovias federais. Foram utilizados os valores para rodovias federais, pois são os únicos que consideram os custos associados às pessoas;
...
$$
    Custo_{2023} = Óbitos_{2023} \times Custo_{2014} \times (1 + IPCA)
$$
- Têm-se um **custo por óbito** de `r custo_individual` e um **custo total** de `r custo_rs`; ou aproximadamente `r format_billion(custo_2023)` dos óbitos em 2023.
