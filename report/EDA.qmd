...
## Calculando mortes por 100.000 habitantes e juntando no dataframe
...
df_total |> 
  ...
  melt() |> 
  ggplot(aes(x = value)) +
    geom_histogram(bins = 15, color = "black", fill = obs_palette$red) +
...
```{r}
mortes_hab |> 
  ggplot(aes(x = ano, y = mortos_por_pop)) +
    geom_line() +
    geom_point() +
    scale_y_continuous(limits = c(0,NA)) +
    scale_x_continuous(breaks = seq(1996,2021,1)) +
